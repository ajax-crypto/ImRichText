...
namespace ImRichText
{
    ...
    [[nodiscard]] ImVec2 GetSegmentSize(const SegmentData& segment, const std::vector<StyleDescriptor>& styles, const RenderConfig& config)
    {
        ...
        if (segment.StyleIdx == -1) return { 0.f, 0.f };
    ...
    void PushTag(std::string_view currTag, int styleIdx)
    {
        CurrentStackPos++;
    ...
    {
        if (reset) TagStack[CurrentStackPos].tag = "";
        --CurrentStackPos;
    }
    ...
    void SetImplicitStyleProps(TagType tagType, std::string_view currTag, 
        StyleDescriptor& styleprops, const StyleDescriptor& parentStyle,
        DrawableLine& line, int propsChanged, const RenderConfig& config)
    {
        ...
        {
            styleprops.blink = true;
            propsChanged = propsChanged | StyleBlink;
        }
        else if (tagType == TagType::Meter)
        {
...
#define DrawDebugRect(...)
    ...
    struct DefaultTagVisitor final : public ITagVisitor
    {
        ...
        bool Attribute(std::string_view name, std::optional<std::string_view> value)
        {
            ...
            currstyle.propsSpecified = RecordTagProperties(tagType, name, value, currstyle, parentStyle, config);
            return true;
        }
        ...
        {
            const auto& parentStyle = Style(CurrentStackPos - 1);
            SetImplicitStyleProps(tagType, currTag, currstyle, parentStyle, currline, currstyle.propsSpecified, config);
            if (IsStyleSupported(tagType) && currstyle.propsSpecified != 0) 
                result.StyleDescriptors.emplace_back(currstyle);
            ...
            if (currline.Segments.empty() || tagType == TagType::Blink)
                AddSegment(currline, styleIdx, result.StyleDescriptors, config);
        ...
        bool TagEnd(std::string_view tag, bool selfTerminatingTag)
        {
            ...
            const auto& styleprops = Style(CurrentStackPos);
            styleIdx = CurrentStackPos >= 0 ? TagStack[CurrentStackPos].styleIdx : -1;
                ...
                tagType == TagType::CodeBlock || tagType == TagType::Marquee)
            {
                ...
                else if (tagType == TagType::Header)
                {
                    ...
                    auto& style = result.StyleDescriptors[currline.Segments.back().StyleIdx + 1];
                    style.height = 1.f;
