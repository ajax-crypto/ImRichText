...
namespace ImRichText
{
    ...
    [[nodiscard]] ASCIITextShaper* ASCIITextShaper::Instance()
    {
        static ASCIITextShaper shaper;
        return &shaper;
    }

    [[nodiscard]] std::tuple<bool, bool, std::string_view> ASCIITextShaper::AddEscapeSequences(const std::string_view content,
        int& curridx, char escapeStart, char escapeEnd)
    {
        ...
        bool ignoreLineBreaks, bool ignoreEscapeCodes, void* userdata)
    {
        ...
        while (to < (int)content.size())
        {
            ...
            {
                to++;
                auto [hasEscape, isNewLine, substitute] = AddEscapeSequences(content, to,
                    config.EscapeSeqStart, config.EscapeSeqEnd);
